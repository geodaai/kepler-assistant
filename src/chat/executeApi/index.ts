 ...
 *
 * Both dispatch through a `ChatToolSurface` (see `handleCommands.ts`) so the
 * harness never depends on the concrete kepler registry. No command logic is
 * duplicated.
 */
...
- executeCommand: Execute an application command by id. Args: commandId and optional input matching the command's input schema.
  ...
  - "map.load-data": Load a dataset from a URL into kepler.gl. The dataset is named after the URL filename, or after the optional \`datasetName\`. Does NOT create a layer — call map.add-layer to visualize. input: { url, datasetName? }. NEVER create a duplicate dataset (e.g. via map.create-table with a SELECT *) just to rename a loaded dataset — name it with \`datasetName\` here, or use the URL-filename dataset as-is.
  ...
  - "map.add-column": Add a NEW column to an EXISTING kepler.gl dataset, in place, with EXACTLY ONE of two value sources. (a) copyFromColumn: copy the values of an existing column — this is how you "rename" a column in place: to rename "fare" to "fare_amount", add a new column "fare_amount" that copies "fare" (the original column stays). (b) expression: an SQL expression computed per row, e.g. a z-score — input: { datasetName, newColumnName, expression: "(HR60 - AVG(HR60) OVER()) / STDDEV(HR60) OVER()" } adds HR60_Z to the existing dataset. CHANGING A COLUMN'S TYPE: use expression to add a cast column, e.g. { datasetName, newColumnName: "NOSOUTH_str", expression: "NOSOUTH::VARCHAR" } converts a numeric column to string (the original column stays). The dataset keeps its name/id/color and existing layers, filters and tooltips are untouched. ONLY adds columns — it cannot delete or rename-in-place; use map.create-table (which builds a NEW dataset, original untouched) if columns must be removed. Prefer map.add-column over creating a new dataset whenever the user asks to add a variable to, rename a column in, or change a column type in an EXISTING dataset.
 ...
 * in the demo-app is provided as that surface — see `mcp/chat-surface.ts`).
 */
export function createExecuteApiTool(surface: ChatToolSurface) {
  const {listCommandsHandler, executeCommandHandler} = buildCommandHandlers(surface);
  ...
  return tool({
    ...
    execute: async (
      {call}: {call: {apiName: string; args: unknown}},
