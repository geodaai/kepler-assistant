 ...
 *
 * The `map.*` commands come from the published `@kepler.gl/mcp` map surface
 * (owned by the kepler.gl monorepo); the analysis shims (`data.*`, `geo.*`,
 * `geoda.*`, `chart.*`) live here and delegate compute to the shared
 * `AnalysisEngine`.
 *
