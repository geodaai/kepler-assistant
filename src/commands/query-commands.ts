...
/**
 * Run the resolved SQL through the shared analysis engine (which executes it
 * against the same DuckDB connector `loadTableIntoDuckDB` materialized the
 * table into) and return ALL rows. `full: true` keeps the model-facing preview
 * uncapped so the persisted result isn't truncated to the engine's 50-row cap.
 */
async function runQueryRows(sql: string): Promise<Record<string, unknown>[]> {
  const result = await runAnalysis('data.query', {sql, full: true});
  ...
  // Normalize any Arrow bigint / proxy values into plain JS primitives before
  // the rows are persisted to DuckDB (mirrors arrowTableToObjects).
  return rows.map(r => convertArrowRowToObject(r));
}
...
IMPORTANT: Use __TABLE_A__ and __TABLE_B__ as table name placeholders in SQL. They will be replaced with actual DuckDB table names at runtime.`,
    ...
    execute: async (_execCtx, input) => {
      ...
      try {
        ...
        await loadTableIntoDuckDB(getValues, datasetNameA, columnNamesA, dbTableNameA);
...
Use this command after running a query (genericQuery, tableCommand, mergeTablesCommand) to visualize the result on a kepler.gl map.`,
    ...
    execute: async (_execCtx, input) => {
      ...
      try {
        // The engine's data.load-to-map pushes the table into kepler through the
        // host KeplerBridge (`loadToMap` → loadTableToKepler).
        const result = await runAnalysis('data.load-to-map', {table: datasetName});
        if (!result.success) {
          return {success: false, commandId: 'data.load-to-map', error: result.error};
        }
        return {
          success: true,
